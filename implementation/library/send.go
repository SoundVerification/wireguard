...
//@ requires pl.token(t) && io.e_out(t, rid, m) && by.gamma(m) == Abs(packet)
//@ ensures  acc(LibMem(libState), 1/16) && acc(Mem(packet), 1/16)
//@ ensures  ok  ==> pl.token(t1) && t1 == old(io.get_e_out_t1(t, rid, m))
//@ ensures  !ok ==> t1 == t && pl.token(t) && io.e_out(t, rid, m) && io.get_e_out_t1(t, rid, m) == old(io.get_e_out_t1(t, rid, m))
