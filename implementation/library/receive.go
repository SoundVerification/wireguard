...
//@ requires pl.token(t) && io.e_in(t, rid)
//@ ensures  acc(LibMem(libState), 1/16)
//@ ensures   ok ==> Mem(packet) && by.gamma(term) == Abs(packet)
//@ ensures   ok ==> pl.token(t1) && t1 == old(io.get_e_in_t1(t, rid)) && term == old(io.get_e_in_m(t, rid))
//@ ensures  !ok ==> t1 == t && pl.token(t) && io.e_in(t, rid) && io.get_e_in_t1(t, rid) == old(io.get_e_in_t1(t, rid)) && io.get_e_in_m(t, rid) == old(io.get_e_in_m(t, rid))
...
//@ ensures   ok ==> pl.token(t1) && t1 == old(io.get_e_Message_t1(t, rid)) && term == old(io.get_e_Message_m(t, rid))
//@ ensures  !ok ==> t1 == t && pl.token(t) && io.e_Message(t, rid) && io.get_e_Message_t1(t, rid) == old(io.get_e_Message_t1(t, rid)) && io.get_e_Message_m(t, rid) == old(io.get_e_Message_m(t, rid))
