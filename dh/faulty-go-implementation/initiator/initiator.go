...
/////////////////////////////////////////////////////////////
// Note that this is a fault implementation, i.e. contains //
// a bug. In particular, this implementation tries to send //
// its DH secret key instead of its DH public key in the   //
// first message. Thus, a verification failure is expected //
// because the I/O specification does not permit this send //
// operation.                                              //
/////////////////////////////////////////////////////////////
...
func (i *Initiator) sendMsg1(x []byte /*@, ghost skAT tm.Term, ghost skBT tm.Term, ghost xT tm.Term, ghost t pl.Place, ghost ridT tm.Term, ghost s mset[ft.Fact] @*/) (err error /*@, ghost t1 pl.Place, ghost s1 mset[ft.Fact] @*/) {
	...
	/////////////////////////////////////////////////////////////
	// faulty implementation trying to send `x` instead of `X` //
	// i.e. the DH secret key instead of DH public key         //
	msg1 := &Msg1{X: x}
	/////////////////////////////////////////////////////////////
	...
	/////////////////////////////////////////////////////////////
	// sending msg1 containing `x` instead of `X` i.e. the     //
	// DH secret key instead of DH public key, results in a    //
	// verification error on the following line, because we    //
	// only have I/O permissions to send `X` but not to send   //
	// `x`.                                                    //
